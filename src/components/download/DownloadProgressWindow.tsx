...
import { cancelSongDownload, openDownloadDirectory } from '../../services/songDownloadService';
...
const DownloadProgressWindow: React.FC<DownloadProgressWindowProps> = ({ isDaylight }) => {
    ...
    const finished = items.filter((item) => item.status === 'done' || item.status === 'error' || item.status === 'canceled');
    ...
    if (!visible) {
        ...
        return createPortal(
            <button
                ...
                className="fixed bottom-24 right-4 z-[290] flex items-center gap-2 rounded-full px-3.5 py-2 text-xs font-semibold shadow-lg backdrop-blur-xl border border-white/15 bg-black/60 text-white hover:bg-black/70 transition-colors"
            >
                <Loader2 size={13} className="animate-spin" />
                {t('download.activeCount', { count: active.length })}
            </button>,
            document.body,
        );
    }
        ...
        >
            <div className="space-y-2">
                <div className="flex items-center justify-between text-[11px] opacity-70">
                    <span>{t('download.progress', { done: finished.length, total: items.length })}</span>
                    <span className="font-mono">
                        {Math.round((finished.length / Math.max(items.length, 1)) * 100)}%
                    </span>
                </div>
                        ...
                        return (
                            <div key={item.id} className="rounded-lg bg-white/[0.04] px-2.5 py-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    {(item.status === 'downloading' || item.status === 'resolving') && (
                                        <button
                                            onClick={() => void cancelSongDownload(item.id)}
