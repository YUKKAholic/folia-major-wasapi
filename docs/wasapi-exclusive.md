...
- **音源**：本地文件始终支持。在线（网易/QQ/酷狗）与 Navidrome 的远程 http(s) 源为**实验性、默认关闭**（设置里的「在线音源也使用独占」）——引擎需先整首下载，会与 Chromium 的在线流抢带宽并可能卡顿，且设备不支持其采样率时必然失败；一旦某次在线独占失败，本会话不再对在线曲尝试。若在线曲命中缓存、只有 `blob:` 地址（主进程拿不到），该曲也走共享模式。
